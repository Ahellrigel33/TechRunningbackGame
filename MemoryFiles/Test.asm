...
; BlinkLED.asm
; This code blinks the right-most decimal place at 1 Hz
; Andrew Hellrigel
; ECE 2031 L09
; 10/12/2020
...
ORG 0
	CALL  SetupI2C
Start:
	; Read in the X value and store it in L2A
	Call  ReadX
	Store L2A
	; Read in the Y value and store it in L2B
	Call  ReadY
	Store L2B
	
	; Compute the L2 Norm and output value
	CALL  L2Estimate
	Shift -4
	OUT   HEX0
	OUT   LEDs
...
;Start:
	; Turn on the LED then wait 0.5 seconds
	LOADI &B000001
	OUT  DPs
	CALL WasteTime
	
	; Turn off the LED then wait 0.5 seconds
	LOADI &B000000
	OUT  DPs
	CALL WasteTime
...
; Runs 11 outer loops * 50505 inner loops * 3 instructions = 1666665 instructions
; In reality there are slightly more instructions for setup but it is negligible
WasteTime:
	; Set up initial variables to run outer loop 11 times
	Load WasteTimeOuterStart
	STORE WasteTimeOuterCount
	CALL WasteTimeOuter
	RETURN
WasteTimeOuter:
	; Set up inital variables to run inner loop 50505 times
	LOAD WasteTimeInnerStart
	CALL WasteTimeInner
	LOAD WasteTimeOuterCount
	ADDI 1
	STORE WasteTimeOuterCount
	JZERO WasteDone
	JUMP WasteTimeOuter
WasteTimeInner:
	; Run inner loop 50505 times
	ADDI 1
	JZERO WasteDone
	JUMP WasteTimeInner
WasteDone:
...
WasteTimeOuterStart: DW &HFFF5
WasteTimeOuterCount: DW 0
WasteTimeInnerStart: DW &H3AB7
...
ReadY:
	...
	LOAD   I2CRCmd     ; load read command
	OUT    I2C_CMD     ; send read command to I2C_CMD register
	LOAD   AccYAddr    ; load ADXL345 register address for Y acceleration 
	OUT    I2C_DATA    ; send to I2C_DATA register
	OUT    I2C_RDY     ; start the communication
	CALL   BlockI2C    ; wait for it to finish
	IN     I2C_data    ; put the data in AC
